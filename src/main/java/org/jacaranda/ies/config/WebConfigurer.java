...
public class WebConfigurer implements ServletContextInitializer, WebServerFactoryCustomizer<WebServerFactory> {
    ...
    private void setMimeMappings(WebServerFactory server) {
        if (server instanceof ConfigurableServletWebServerFactory) {
            ...
            // Boot 3's Undertow defaults no longer expose the legacy .abs mapping.
            // Keep the static-asset MIME contract that the application previously served.
    ...
    public CorsFilter corsFilter() {
        ...
        if (
            ...
            // Spring Framework 5.3 rejects "*" with credentials. Preserve the legacy
            // wildcard behavior by using the dedicated origin-pattern mechanism.
            config.setAllowedOriginPatterns(config.getAllowedOrigins());
